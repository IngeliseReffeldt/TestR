...
shap_values <- fastshap::explain(
  predictor_rf$model,
  X = task$data(rows = split$test), # All rows; maybe it is to much? For fewer rows: task$data(rows = split$test)[1:10,]
...
Shapviz object
```{r}
sv <- shapviz::shapviz(shap_values, X = task$data(rows = split$test)[1:10,])
```
Beeswarm plot
```{r}
